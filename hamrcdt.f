      program hamrcdt
c
c     revised 2023-05-27
c
c       multiple transitions
c
c     *interlayer exchange coupling added
c     *tc can be different for different layers
c     *tmperature can be different for different layers
c
c
c     head field rise time included
c
      implicit real(a-h,o-z),integer(i-n)
      parameter (mpq=2000,mz=50,mq=mz,mq3=3*mq,mt=200000
     +        ,mw=100,lr=20*mq3+130,li=200,my=500,mx=500)
c
      dimension  y(mq3),yo(mq3),rw(lr),iw(li),nqt(1),rto(1),ato(1)
     +          ,zp(mz),yp(mpq),arz(mz),nnv(mpq),vx(mpq,mz),vy(mpq,mz)
     +          ,ymx(mpq,mz),ymy(mpq,mz),ymz(mpq,mz)
     +          ,heb(mz),ztmp(mt,mpq)
     +          ,tempc(mz),gms(mz),hko(mz)
     +          ,ahko(mz),aexo(mz),ept(mz)
     +          ,avgmz(mpq),mvalue(8),tc(mpq,mz)
     +          ,gpx(mpq),gpy(mpq),gpz(mpq),gps(mpq)
     +          ,ttw(mt),hhw(mt),tmpt(mz),bms(mz)
     +          ,amgx(mpq),amgy(mpq),amgz(mpq)
     +          ,amdx(mw,mpq),amdy(mw,mpq),amdz(mw,mpq)
c
      common /pfa/  nz,nqs,nqs3
      common /pfb/  alfg(mz),alft(mz)
      common /pfc/  huk(mz),hea(mz)
      common /pfd/  htx(mz),hty(mz),htz(mz)
      common /pfe/  hax(mz),hay(mz),haz(mz)
      common /msk/  amk(mz)
      common /grn/  gsx(mpq),gsy(mpq)
c
      common /ran/  R(97), IX1, IX2, IX3
c
      common /ptf/  atpx(mz,my,mx),atpy(mz,my,mx),atpz(mz,my,mx)  
      common /ptg/  atpt(mz,my,mx),tpf(mz,my,mx)
      common /ptp/  ntpx,ntpy,ntpz,gymax
c
c
      character*18   fo_hfd,fo_dym,fo_avg,fo_vec,fo_tmp
      character*12   pat
      character*19   pat_pos
      character*8    fmed
      character*14   fpfx
      character*12   fi_med
      character*5    chmy
      character*1    cha, chtmp
      character*3    pix
      character*6    arg1
      character*8    arg2
      character*12   fpa_in
...
c
      nargs=iargc()
      if(nargs.ne.2) stop 'hamrcdt arg1(6) arg2(8) '
      call getarg(1,arg1)
      call getarg(2,arg2)
      read(arg1(5:6),'(i2)') nrm
      read(arg1(4:4),'(i1)') jgp
      jgp=jgp+1
      fpfx=arg1//arg2
      fpa_in=arg2//'.par'
c
      ...
      write(*,*) "___    "
      write(*,*) "Chosen Grain Pattern ", jgp
      write(*,*) "___    "
c
      open(unit=8,file=fpa_in,status='old')
      write(*,*) "Reading the parameter file"
c
c
      alimt=3.0
c
      read(8,*) tpc_avg, tcsigma, dtc 
      read(8,*) tambient
      read(8,*) spd, pmove, pstart
      read(8,*) ams_rmt, hk_rmt, hk_sigma
      read(8,*) gsz, asz, nz
      read(8,*) dtsimi, antf
      read(8,*) hapi, hangle, trise
      read(8,*) sbit, sdelay
      read(8,*) npwrite, nmwrite, ntwrite
      read(8,*) ichki
      read(8,101) fmed 
c
      fi_med=fmed//'.med'
c
      write(*,*) "   "
      write(*,*) "Finished Reading Parameter File"
      write(*,*) " "
c
c     nqs = number of layers in the grain
c    
c
      amso=ams_rmt/((1.0-300.0/tpc_avg)**(1.0/3.0))
c
c
c
...
c
c
c         Read-in thermal profile
c
c
cccccccccccccccccccccccccccccccccccccccccccccc
c
c
      open(unit=23,file='tpf3d14a',status='old')
      read(23,*) ntpx, ntpy, ntpz, ndumb
      do 188 jz=1,ntpz
        do 186 ky=1,ntpy
          do 182 ix=1,ntpx
            read(23,*,end=190) atpx(jz,ky,ix)
     +               , atpy(jz,ky,ix) 
     +               , atpz(jz,ky,ix) 
     +               , atpt(jz,ky,ix) 
 182      continue
 ...
 188  continue
c
 190  write(*,*) "unexpected ending"
c
      close(unit=23)
c
      write(*,*) "Thermal Profile"
      write(*,*)  ntpx, ntpy, ntpz 
      write(*,*) "Finished Reading Thermal File"
      write(*,*) " "
c
c Head-Medium Spacing
c
      hms=atpz(1,1,1)
c
c     Top Co Layer
c
      do jz=1,nz
        zp(jz)=hms+asz*float(jz-1)+0.0001
      enddo
c
      do 199 jz=1,nz
        kzs=1
        kzt=2
 192    if(zp(jz).lt.atpz(kzt,1,1)) then
          if(kzt.lt.ntpz) then
            kzs=kzs+1
            kzt=kzs+1
            go to 192
        ...
        if(kzt.le.ntpz) then
          do 195 ky=1,ntpy
            do 194 ix=1,ntpx
              ...
              atz1=atpz(kzs,ky,ix)
              atz2=atpz(kzt,ky,ix)
              tpf(jz,ky,ix)=tmp1+(tmp2-tmp1)
     +            *(zp(jz)-atz1)/(atz2-atz1)
 194        continue
 195      continue
        else
          do 197 ky=1,ntpy
            do 196 ix=1,ntpx
              tpf(jz,ky,ix)=atpt(kzt,ky,ix)
 196        continue
 197      continue
        endif
 199  continue
c
      write(*,*) "Head Field   ", hapi
      pi=4.0*atan(1.0)
      qi=pi/180.0
c
c
      gvm=gsz*gsz*asz*(1.0e-21)
c
      gszn=gsz
      gvmn=gszn*gszn*asz*(1.0e-21)
c
      akb=1.38e-16
      akbtc=akb*tpc_avg
      hnm=akbtc/(gvmn*amso)
c
      write(*,*) " "
      write(*,*) " Normalization field hnm ", hnm
      write(*,*) " "
c
      haxo=0.0
      hayo=hapi*sin(hangle*3.14159/180.0)
      hazo=hapi*cos(hangle*3.14159/180.0)
c
      haxi=haxo/hnm
      hayi=hayo/hnm
      hazi=hazo/hnm
c
      open(unit=25,file=fi_med,status='old')
      do 191 jz=1,nz
        read(25,*) iz,tempc(jz),ahko(jz),aexo(jz),ept(jz)
     +          ,alfg(jz),alft(jz)  
        if(jz.ne.iz) then
          write(*,*) "Faulty reading in medium file"
          go to 90
        endif
 191  continue
      close(unit=25)
c
c     alfg     Gilbert Damping Constant for the LLG equation
c
c     alft     Damping Constant Used for Calculating Thermal Field
c
c
      do jz=1,nz
        heb(jz)=2.0*aexo(jz)/(hnm*amso*asz*asz*1.0e-14)
        hko(jz)=(ahko(jz)/hnm)
     +   /((1.0-300.0/tempc(jz))**(ept(jz)/3.0))
      enddo
c
      write(*,*) "  " 
      write(*,*) "Exchange Field", heb(2)*hnm 
      write(*,*) "  " 
c
c
ccccccccccccccccccccccccccccccccccccccccccccc
c                                           c
c            Input Medium Grains 
c                                           c
ccccccccccccccccccccccccccccccccccccccccccccc
c
c
...
c
      gama=1.76e7/1.0e9
      aaa=2.0*akb
      bbb=amso*gvm*gama*dts
      htho=sqrt(aaa/bbb)/hnm
      write(*,*) " "
      write(*,*) " Thermal Field ", htho*hnm
c     *sqrt(0.05*650/((1.0-600/610)**(1/3)))
      write(*,*) " "
c
c
...
c
c
c     Start read grain pattern files
c
c
ccccccccccccccccccccccccccccccccccccc
c
      do igp=1,jgp
        read(8,101,end=9009) pat
      enddo
      write(*,*) "Grain Pattern File"
      write(*,101) pat
      write(*,*) " "
c
cccccccccccccccccccccccccccccccc
c
c     open grain center position file
c     gps(i) is the area of ith grain in nm^2
c
c
      pat_pos=pat//'_ga.dat'
      open(unit=18,file=pat_pos,status='old')
      indx=1
 2205 read(18,*,end=2206) aindx, gpx(indx), gpy(indx), gps(indx)
      indx=indx+1
      go to 2205
 2206 nps=indx-1
      close(unit=18)
c
      write(*,*) "Input grain area file finished "
c
      gymax=(-1.0e3)
      gymin=(+1.0e3)
      gxmax=(-1.0e3)
...
c 
      do 2210 ik=1,nps
        if(gpy(ik).gt.gymax) then
          gymax=gpy(ik)
        endif
        if(gpy(ik).lt.gymin) then
          gymin=gpy(ik)
        endif
        if(gpx(ik).gt.gxmax) then
          gxmax=gpx(ik)
        endif
        if(gpx(ik).lt.gxmin) then
          gxmin=gpx(ik)
        endif
 2210 continue
c
      gxmid=(gxmax-gxmin)/2.0
      gymid=(gymax-gymin)/2.0
c
      write(*,*) " "
      write(*,*) "X-coord of the Grains:  ", gxmin, gxmax 
      write(*,*) "Y-coord of the Grains:  ", gymin, gymax 
c
      write(*,*) "Number of grains =    ", nps
c
      do 2211 ik=1,nps
        gsx(ik)=gpx(ik)-gxmin
        gsy(ik)=gpy(ik)-gymax
...
c
      do 2216 ik=1,nps
 2212   atmp=gasdev(jdum)
        if(abs(atmp).gt.5.0) then
          go to 2212
        endif  
        rtc=tcsigma*atmp
        do 2215 jz=1,nz
          tc(ik,jz)=tempc(jz)*(1.0+rtc)
 2215   continue
 2216 continue
c
      nqz=nqt(1)+1
...
c
c     The following input file describes edge positions of all grains
c
c
      pat_pos=pat//'_gp.dat'
      open(unit=18,file=pat_pos,status='old')
      indx=1
 2302 read(18,*,end=2308) aindx, gnv
      nnv(indx)=ifix(gnv+0.01)
      do inv=1,nnv(indx)
        read(18,*) tvx, tvy 
        vx(indx,inv)=tvx
        vy(indx,inv)=tvy
 ...
 2308 npsp=indx-1
      close(unit=18)
      if(npsp.ne.nps) then
        write(*,*) "Number of grains does not match    ", nps, npsp
        go to 90
      endif
      write(*,*) " "
      write(*,*) "Input grain edges file finished "
      write(*,*) " "
c
c
...
c
c
c        Initial magnetization patterns
c
cccccccccccccccccccccccccccccccccccccccccccccc
c
c
c
      ...
      if(ichki.eq.0) then
        do 2526 ik=1,nps
          ...
          ra3=ran1(idum)
          if(ra1.gt.0.5) then
            theta=(3.0*ra2)*pi/180.0;
          else
            theta=(180.0-3.0*ra2)*pi/180.0;
          endif
          phi=ra3*2*pi/180.0
          samx=sin(theta)*cos(phi)
          samy=sin(theta)*sin(phi)
          samz=cos(theta)
          do 2523 jz=1,nz
            ymx(ik,jz)=samx
            ymy(ik,jz)=samy
...
c
c
c     Set up simulatio time steps       
c
c       Calculate total simulation time  
c
        ypsiz=gymax-gymin+float(ntpy)
        totsim=pmove*ypsiz/spd
        nht=ifix(totsim/dts)+1;
        write(*,*) "total sim, nht ", totsim, nht
        npwskip=ifix(float(nht)/float(npwrite))
        nmwskip=ifix(float(nht)/float(nmwrite))
        ntwskip=ifix(float(nht)/float(ntwrite))
c
c
...
c
        ...
        sgni=(-1.0)
        do 5028 iht=1,nht
          ttw(iht)=float(iht-1)*dts
          tsm=tsm+dts
          if(tsm.gt.tbit) then
            tsm=0.0
            sgni=sgni*(-1.0)
          endif
          hhw(iht)=sgni*(2.0*exp((-1.0)*tsm/trise)-1.0)
 5028   continue
c
c
c        open(unit=11,file=fo_hfd)
c        do 5029 iht=1,nht
c          write(11,*) ttw(iht), hhw(iht)
c 5029   continue
c        close(unit=11)  
c
c
...
c       Start Simulation
c
c       Scan through each grains
c       Each grain gets moved at speed: spd
c
...
c
          tsim=ttw(iht)
c
          asy=bsy+tsim*spd+pstart
c
          call caltemp(nz,tmpt,asx,asy)
c
          if(mod(iht,npwskip).eq.0) then
            itp=itp+1
            ztmp(itp,ik)=tmpt(1)
          endif
c
          do 5102 jz=1,nz
            ...
            jmz=(jz-1)*3+3
            tmpc=tc(ik,jz)
            if(tmpt(jz).gt.(tmpc-dtc)) then
              theta=3.14159*ran1(idum)
              phi=2.0*3.14159*ran1(idum)
              ssp=sin(phi)
              csp=cos(phi)
              sst=sin(theta)
              cst=cos(theta)
              ...
              hea(jz)=0.0
              amk(jz)=0.0
            elseif(tmpt(jz).lt.tambient) then
              htx(jz)=0.0
              hty(jz)=0.0
              htz(jz)=0.0
              hax(jz)=0.0
              ...
              bms(jz)=((1.0-tmpt(jz)/tmpc)**(1.0/3.0))
              hthi=htho*sqrt(alft(jz)*tmpt(jz)/bms(jz))
c
 506          atmp=gasdev(jdum)
              if(abs(atmp).gt.3.0) then
                go to 506
              endif
c
              theta=3.14159*ran1(idum)
              ...
              sst=sin(theta)
              cst=cos(theta)
              htx(jz)=hthi*atmp*sst*csp
              hty(jz)=hthi*atmp*sst*ssp
              htz(jz)=hthi*atmp*cst
c
              hax(jz)=hhw(iht)*haxi
              hay(jz)=hhw(iht)*hayi
              haz(jz)=hhw(iht)*hazi
...
c
          ...
          dtf=tf/antf
 5106     to=t+dtf
          call slsode(fun,nqt,y,t,to,ito,rto,ato,ita,ist
     +               ,iop,rw,lr,iw,li,jac,mf)
          if(ist.lt.0) then
            call outp(ist)
            write(*,*) "I was hoping not to end up here!"
            go to 90
          endif
          do 5108 jz=1,nz
            jmx=(jz-1)*3+1
            jmy=(jz-1)*3+2
            jmz=(jz-1)*3+3
            yamp=sqrt(y(jmx)*y(jmx)+y(jmy)*y(jmy)+y(jmz)*y(jmz)) 
            ydif=abs(yamp-1.0)
            if(ydif.gt.0.02) then
              write(*,*) "Yamp Anormny!", ydif
              ist=1
            endif
            y(jmx)=y(jmx)/yamp
            y(jmy)=y(jmy)/yamp
...
c
c
c         finish one dts step
c
          if(mod(iht,nmwskip).eq.0) then
            iwr=iwr+1
            call myavg(y,nq,nz,amx,amy,amz)
            amdx(iwr,ik)=amx
            amdy(iwr,ik)=amy
            amdz(iwr,ik)=amz
          endif
c
          if(mod(iht,ntwskip).eq.0) then
            write(*,*) tsim, totsim, ik, nps
          endif
c
          if(iht.lt.nht) then
            go to 5100
          endif
          write(*,*) "Motion completed for Grain", ik
c
c         Finish all steps for a single grain
c
          call myavg(y,nq,nz,amx,amy,amz)
          amgx(ik)=amx
          amgy(ik)=amy
...
c
 5298   continue
c
 9009 write(*,*) "All grains finished !!! ",  nps
c
      fo_vec='vec_'//fpfx
      open(unit=3,file=fo_vec)
      write(3,*) nps, nht, nht
      do 9018 ik=1,nps
        write(3,*) ik, nnv(ik), nps
        do 9016, iv=1,nnv(ik)
          write(3,*) vx(ik,iv), vy(ik,iv), ze
 9016   continue
 9018 continue
      do 9020 ik=1,nps
        write(3,*) amgx(ik), amgy(ik), amgz(ik)
 9020 continue
      close(unit=3)
c
      fo_dym='dym_'//fpfx
      open(unit=7,file=fo_dym)
      write(7,*) nps, nwr, nht
      do 9518 ik=1,nps
        write(7,*) ik, nnv(ik), nps
        do 9516, iv=1,nnv(ik)
          write(7,*) vx(ik,iv), vy(ik,iv), ze
 9516   continue
 9518 continue
c
      do 9525 iwr=1,nwr
        write(7,*) iwr, nwr, nps
        do 9520 ik=1,nps
          write(7,*) amdx(iwr,ik), amdy(iwr,ik), amdz(iwr,ik)
 9520   continue
 9525 continue
c
...
c
      fo_tmp='tmp_'//fpfx
      open(unit=9,file=fo_tmp)
      write(9,*) nps, ntmpw, nht
      do 9032 ik=1,nps
        write(9,*) ik, nnv(ik), nps
        do 9030, iv=1,nnv(ik)
          write(9,*) vx(ik,iv), vy(ik,iv), ze
 9030   continue
 9032 continue
      do 9038 itp=1,ntmpw
        write(9,*) itp, ntmpw, nht
        do 9036 ik=1,nps
          write(9,*) ztmp(itp,ik), ztmp(itp,ik), ztmp(itp,ik)
 9036 continue
 9038 continue
c
c
      ...
      close(unit=9)
c
 109  format(f11.3," ",f6.3," ",f6.3," ",f6.3," "
     +       ,f6.3," ",f6.3," ",f6.3)
c
  90  stop
      end
c
      subroutine  myavg(y,nq,nz,amx,amy,amz)
      implicit real(a-b,d-h,o-z),integer(i-n),complex(c)
      dimension y(nq)
      amx=0.0
      amy=0.0
      amz=0.0
...
c
c
      subroutine caltemp(nz,tmpt,asx,asy)
      implicit real(a-b,d-h,o-z),integer(i-n),complex(c)
      parameter (mpq=2000,mz=50,mq=mz,mq3=3*mq,mt=10000
     +          ,lr=20*mq3+130,li=200,my=500,mx=500)
c
      dimension tmpt(nz)
c
      common /ptf/  atpx(mz,my,mx),atpy(mz,my,mx),atpz(mz,my,mx)  
...
c
      ...
      do 5 jz=1,nz
        tmpt(jz)=400.0
 5    continue
      if(ksy.ge.1.and.ksy.le.ntpy) then
        if(ksx.ge.1.and.ksx.le.ntpx) then
          do 10 jz=1,nz
            tmpt(jz)=tpf(jz,ksy,ksx)
 10       continue
        endif
      endif
      return
...
c
      subroutine dropout(n)
      implicit real(a-b,d-h,o-z),integer(i-n),complex(c)
      integer n
      if(n.eq.1) then
         write(*,*) "nx is too large"
      elseif(n.eq.2) then
         write(*,*) "ny is too large"
      else
         write(*,*) "nz is too large"
      endif
      return
      end
c
      ...
      implicit real(a-b,d-h,o-z),integer(i-n),complex(c)
      integer ist
      open(unit=10,file='errout',status='unknown')
      write(10,*) "ist = ", ist
      write(10,*) "program abort!"
      return
      end
c
c
c
      subroutine  fun(nqt, t, y, yd)
      implicit real(a-h,o-z),integer(i-n)
      parameter (mpq=2000,mz=50,mq=mz,mq3=3*mq,mt=10000)
c
c
c
      dimension  y(mq3),yd(mq3),agm(mq),nqt(1)
     +          ,hanx(mq),hany(mq),hanz(mq)
     +          ,ahx(mq),ahy(mq),ahz(mq)
     +          ,afx(mq),afy(mq),afz(mq)
     +          ,agx(mq),agy(mq),agz(mq)
     +          ,spx(mq),spy(mq),spz(mq)
     +          ,hex(mq),hey(mq),hez(mq)
...
c
        if(jz.eq.1) then
          hex(jz)=hea(jz)*y(jmxp)
          hey(jz)=hea(jz)*y(jmyp)
          hez(jz)=hea(jz)*y(jmzp)
        elseif(jz.eq.nz) then
          hex(jz)=hea(jzn)*y(jmxn)
          hey(jz)=hea(jzn)*y(jmyn)
          hez(jz)=hea(jzn)*y(jmzn)
        else
          hex(jz)=hea(jz)*y(jmxp)+hea(jzn)*y(jmxn)
          hey(jz)=hea(jz)*y(jmyp)+hea(jzn)*y(jmyn)
          hez(jz)=hea(jz)*y(jmzp)+hea(jzn)*y(jmzn)
        endif
c
        hanx(jz)=0.0
        hany(jz)=0.0
        hanz(jz)=huk(jz)*y(jmz)
c
        ahx(jz)=hanx(jz)+hax(jz)+hex(jz)
        ahy(jz)=hany(jz)+hay(jz)+hey(jz)
        ahz(jz)=hanz(jz)+haz(jz)+hez(jz)
c
c
        agx(jz)=alfg(jz)*(ahx(jz)+htx(jz))
        agy(jz)=alfg(jz)*(ahy(jz)+hty(jz))
        agz(jz)=alfg(jz)*(ahz(jz)+htz(jz))
c
        spx(jz)=ahy(jz)*y(jmz)-ahz(jz)*y(jmy)
        spy(jz)=ahz(jz)*y(jmx)-ahx(jz)*y(jmz)
        spz(jz)=ahx(jz)*y(jmy)-ahy(jz)*y(jmx)
c
        agm(jz)=agx(jz)*y(jmx)+agy(jz)*y(jmy)+agz(jz)*y(jmz)
c
        yd(jmx)=amk(jz)*(spx(jz)+agx(jz)-agm(jz)*y(jmx))
        yd(jmy)=amk(jz)*(spy(jz)+agy(jz)-agm(jz)*y(jmy))
        yd(jmz)=amk(jz)*(spz(jz)+agz(jz)-agm(jz)*y(jmz))
c
 210  continue
c
      return
...
c
      FUNCTION RAN1(IDUM)
      implicit real(a-b,d-h,o-z),integer(i-n),complex(c)
      common /random1/ R(97),IX1,IX2,IX3
...
ccccccccccccccccccccccccccccccccccccccccccc
c
c     gamma=0.0176 (1/Oe)(1/naosecond)
c
ccccccccccccccccccccccccccccccccccccccccccc
c
c
...
C                               real array of length 9 or more, and
C                               ISAV must be an integer array of length
C                               25 or more.  JOB = 1 means save COMMON
...
C     (2) the internal COMMON block /SLS001/, of length 34 
C         (9 single precision words followed by 25 integer words).
C
...
C Subroutines SLSODE, SINTDY, SSTODE, SPREPJ, and SSOLSY.
C-----------------------------------------------------------------------
...
C*Internal Notes:
C-----------------------------------------------------------------------
C Subroutines/functions called by RUMACH.. None
C-----------------------------------------------------------------------
C**End
C
      REAL U, COMP
